...
The server also runs a projector that reads the append-only JSONL official
...
```powershell
cargo run -p ham-server --bin ham-server -- --rebuild-projection
```
...
`ham-client` instance requires that instance to run with
