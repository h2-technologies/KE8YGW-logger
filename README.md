...
The desktop application and CLI release line is `0.3.0`. Native iOS is a
separate `0.3.1` release and is not coupled to the desktop tag. Shared Rust
crates remain on the repository's `0.2.0` compatibility line unless a crate is
a product package (`ham-cli`, `ham-desktop`, or the Tauri package). CI enforces
these deliberately different release versions with `scripts/check_versions.py`.
