...
- Apple multicast entitlement declaration is configured in the iOS project;
  Apple Developer account approval/provisioning and physical-device validation
  remain beyond the automatic
...
Native iOS status:
- Native SwiftUI, Rust FFI, Xcode project, multicast entitlement file, Apple build scripts, and iOS workflow files are present as of July 22, 2026.
...
- A persisted projection (today: the SurrealDB projection the `ham-sync`
  projector maintains) is a disposable copy, never a source of truth. Data flows
  one way, official log to projection; nothing writes to the official log as a
  result of reading a projection. Only the projector writes the projection
  tables. Tombstones project as projection-level removal, never as a row delete
  that could be mistaken for deleted history. Entries are hash-chain verified
  before they are projected, and a broken chain halts the projector at the break
  instead of projecting past it. See
...
| SurrealDB projection of QSOs and activations | `ham-sync::projector` (row content from `ham-core::projection`) | SurrealDB `projection_*` tables on the sync server, with a durable checkpoint record | No; rebuilt by replaying the official log | Projector replay only; every other component reads it |
...
| Projected tables, checkpoint, projector modes, or rebuild procedure | `docs/PROJECTION_PIPELINE.md` |
...
- Known critical and high-priority gaps:
  ...
  - Physical-device LAN validation for the declared multicast entitlement and end-to-end sync branch-review/reconciliation workflow qualification.
