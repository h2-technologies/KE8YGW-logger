...
- Automatic LAN discovery and production pairing UX beyond the manual direct
  LAN HTTP peer path.
...
- Actual synchronization state: `ham-sync` implements LAN discovery and verification models, preview/pull/push logic, cloud/self-hosted sync models, durable self-hosted sync/report storage, guarded replay rules, durable offline mutation queue models, desktop/iOS queue hooks, queue-aware cloud push acknowledgment, structured conflict reports, durable manual conflict-review records, explicit recovery-path decisions, durable LAN trust records, and GUI manual direct LAN HTTP preview/pull transport. Automatic LAN discovery transport, production pairing UX, corrective-event conflict-resolution UX, and release-device iOS background retry validation are still missing.
...
- Known critical and high-priority gaps:
  ...
  - Automatic LAN discovery transport, production pairing UX, and corrective-event sync conflict-resolution UX.
