...
- Actual hosted-account client state: `ham_sync::account` owns durable client account state, request planning, response classification, and state transitions for the hosted account, session, recovery, and device routes. Desktop drives it through `/api/account/*` GUI endpoints, a `ureq` transport, `CredentialStore`-held tokens, and a browser Account screen; native iOS drives it through `account.*` bridge commands, a `URLSession` transport, and Keychain-held tokens. Planned requests carry credential references, never secrets, and cleartext hosted transport is refused outside loopback/private/link-local hosts. Hosted web has no browser client, administrator hosting/invitation management has no client surface, and neither client surface has been qualified against a production hosted deployment or on release devices.
