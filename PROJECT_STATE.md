...
- Durable offline mutation queue schema and JSON support store in `ham-sync`,
  ...
  plus typed queue health, planned official-event envelope decoding, hosted
  push request construction, and conflict-review display, v0.2 absent/legacy queue migration,
...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  messages in the Sync workspace, and has simulator-safe fallback tests for
  review creation/decoding and selected recovery-path resolution. Production
  iOS reciprocal LAN pairing UX, stronger LAN
