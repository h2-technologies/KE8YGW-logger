...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  through `sync.remote_events.apply` without owning event validation. The iOS
  ...
  sync token are present, and the Rust queue snapshot reports pending work, and
  the task handler delegates to the existing Rust-plan -> Swift-transport ->
  Rust-result retry executor.
  ...
  divergence result recording, background retry scheduling policy boundaries,
