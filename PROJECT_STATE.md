...
| #25 Operational limits, request IDs, audits, safe errors | Satisfied for server foundation | `HostedLimitConfig`, `RateLimitRecord`, request ID success/error propagation, `AuditRecord`, provider/sync/account limit enforcement, stable error codes in `ham_core::api_contract`, `tests::request_ids_limits_and_audits_are_durable_and_redacted` |
...
external blocker. `ham_core::sync::account` already provides the planning,
