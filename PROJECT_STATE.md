...
- Durable offline mutation queue schema and JSON support store in `ham-sync`,
  ...
  plus typed queue health, planned official-event envelope decoding,
...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  messages in the Sync workspace. Native iOS can plan through Rust, execute the
  configured sync-token push path through Swift transport, split an accepted
  server prefix from a rejected tail, and record accepted/auth/divergence
  outcomes back through Rust-owned retry results, and apply pulled official
  ...
  user-action stops without token leakage, pulled-event apply decoding, and
  partial divergence result recording. Production iOS reciprocal LAN pairing
...
validation, native push/pull endpoint qualification, and release-device iOS
background task and poor-network qualification. That goal unblocks
