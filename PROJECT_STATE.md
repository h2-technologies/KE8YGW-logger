...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  server prefix from a rejected tail, record accepted/auth/divergence outcomes
  back through Rust-owned retry results, fetch self-hosted/logbook-scoped pull
  responses through Swift transport, and apply pulled official envelopes through
  `sync.remote_events.apply` without owning event validation.
  ...
  fetch/apply coordination, pulled-event apply decoding, and partial divergence
  result recording. Production iOS reciprocal LAN pairing UX, stronger LAN
