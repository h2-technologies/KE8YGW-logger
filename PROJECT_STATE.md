...
- Durable offline mutation queue schema and JSON support store in `ham-sync`,
  ...
  handling, shared accepted-prefix/rejected-tail queue recovery coverage,
  revoked cloud-auth user-action recovery coverage, and conflict-review
  display, v0.2 absent/legacy queue migration,
...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  discovery packet decoding/peer-URL derivation/identity-probe matching,
  reciprocal LAN pairing request construction and remote-accept validation,
  LAN peer identity mismatch rejection, pulled-event apply decoding, partial
  divergence result recording, shared accepted-prefix/rejected-tail queue
  recovery, durable local identity decoding, and LAN trust
  ...
  networking for paired-device sync. Apple multicast entitlement/provisioning,
...
- Release-device cross-device branch review and reconciliation workflow
  ...
  iOS background task and poor-network validation.
