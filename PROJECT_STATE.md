...
- Sync has discovery, handshake, preview/pull/push verification models, durable
  ...
  issue and acceptance, direct peer trust, LAN auth credential rotation, and
  revocation through FFI commands and a minimal Sync workspace trust section;
  ...
  through Swift transport, sign protected LAN `get-head`/`events-since`
  requests with Keychain-backed auth secrets, and apply pulled official
  envelopes through `sync.remote_events.apply` without owning event validation.
  ...
  networking for paired-device sync. Production iOS reciprocal LAN
...
- Production iOS reciprocal LAN pairing/address-discovery UX, stronger LAN
  ...
  snapshot/issue/accept/trust/rotate/revoke plus signed LAN pull bridge and
...
pairing/address-discovery UX, stronger LAN key-exchange hardening, end-to-end
